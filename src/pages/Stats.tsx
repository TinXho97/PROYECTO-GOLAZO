...
import { getAdvancedStats } from '../services/analyticsProcessing';
...
export default function StatsPage() {
  ...
  const stats = useMemo(() => {
    return getAdvancedStats(bookings, sales, pitches, timeRange);
  }, [bookings, sales, pitches, timeRange]);
  
  const COLORS = ['#0ea5e9', '#3b82f6', '#a855f7', '#f59e0b'];

  const hasChartData = stats?.chartData?.some((d: any) => d.ingresos > 0 || d.reservas > 0) || false;
  const hasDistributionData = stats?.distribution?.some((d: any) => d.value > 0) || false;
  const hasPitchData = stats?.pitchPerformance?.some((p: any) => p.income > 0 || p.occupancy > 0) || false;
        ...
        <CardContent className="p-8 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-gradient-to-br from-sky-500 to-blue-600 p-6 rounded-2xl flex flex-col justify-between">
              ...
              <div className="mt-4">
                <span className="text-3xl font-black text-white">${Math.round(stats?.metrics?.predictedMonthly || 0).toLocaleString()}</span>
          ...
          <Card className="border-none shadow-sm rounded-[32px] overflow-hidden bg-white">
            ...
            <CardContent className="p-8">
              {hasPitchData ? (
                ...
                <div className="w-full py-8 flex flex-col items-center justify-center text-zinc-400 gap-2">
                  <BarChart3 className="w-8 h-8 opacity-20" />
                  <span className="text-xs font-bold uppercase tracking-widest text-center px-4">Sin rendimiento<br/>registrado</span>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-8 h-[250px] flex items-center justify-center relative">
              ...
              <div className="absolute flex flex-col items-center">
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-zinc-400 gap-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-center px-4">Sin datos<br/>registrados</span>
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-none shadow-sm bg-white rounded-3xl">
          <CardContent className="p-6">
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-black text-zinc-900">${Math.round(stats?.metrics?.avgDailyIncome || 0).toLocaleString()}</span>
        ...
        <Card className="border-none shadow-sm bg-white rounded-3xl">
          <CardContent className="p-6">
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-black text-zinc-900">{Math.round(stats?.metrics?.avgOccupancy || 0)}%</span>
              <div className="w-full bg-zinc-100 h-1.5 rounded-full mt-2 overflow-hidden">
                <div 
                  ...
                  style={{ width: `${stats?.metrics?.avgOccupancy || 0}%` }}
                />
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border-none shadow-sm bg-white rounded-3xl">
          <CardContent className="p-6">
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-black text-zinc-900">{stats?.metrics?.mostProfitableHour || 'N/A'}</span>
        ...
        <Card className="border-none shadow-sm bg-white rounded-3xl">
          <CardContent className="p-6">
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-black text-zinc-900">{stats?.metrics?.leastOccupiedHour || 'N/A'}</span>
